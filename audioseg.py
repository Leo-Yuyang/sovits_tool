...
def segaudio(input_file, output_dir):
    ...
    # interval = 15            # The interval of the video in seconds
    interval = random.choice([10, 11, 12, 13, 14, 15])
    ...
    for i, start, stop in tqdm(cut_ranges):
        if not dry_run:
            if (stop - start) / sample_rate > interval:
                # split the file with interval seconds
                tmp_stop = start + interval * sample_rate
                while tmp_stop < stop:
                    ...
                    wavfile.write(
                        ...
                        rate=sample_rate,
                        data=samples[start:tmp_stop],
                    )
                    logger.info((tmp_stop - start) / sample_rate)

                    start = tmp_stop
                    tmp_stop = min(start + interval * sample_rate, stop)
                if (tmp_stop - start) / sample_rate > 4:
                    output_file_path = "{}.wav".format(
                        os.path.join(output_dir, str(uuid.uuid4()))
                    )
                    print("Writing file {}".format(output_file_path))
                ...
                logger.info((stop - start) / sample_rate)
        else:
...
input_dir = "/data/repos/sovits_tool/mix_voice"
output_dir = "/data/repos/sovits_tool/mix_output"
for file in tqdm(os.listdir(input_dir)):
    input_file = os.path.join(input_dir, file)
    segaudio(input_file, output_dir)
